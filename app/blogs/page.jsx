...
const Blogs = () => {
  ...
  useEffect(() => {
    if (active === "all") {
      // If active category is "All", set sortedBlogs to the entire list of blogs
      setSortedBlogs(blogs);
    } else {
      // If active category is not "All", filter blogs based on the active category
      const sorted = blogs.filter((item) => item.category === active);
      ...
      <div className="flex w-full flex-col gap-10 px-[20px] md:px-[100px]">
          ...
          {blogstitle.map((item, index) => {
            return (
              <div
                key={index}
                className={` ${
                  active === item.name ? "bg-[#F49D47]" : " bg-transparent"
        ...
        {sortedBlogs && sortedBlogs.length > 0 ? (
          ...
          <div className="w-full flex items-center justify-center py-20">
            <h1 className="text-[24px] font-[800]">No Event Found</h1>
          </div>
        )}
      </div>
      <Footer />
